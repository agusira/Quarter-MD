...
export class Execute extends Collection {
  constructor (m, sock, { db }) {
    ...
    this.execute = async () => {
      ...
      let count = 1;
      let text = `*DASHBOARD*\n\n`
      text += `( *${count++}* ) Time : ${timers(process.uptime() * 1000)}\n`
      text += `( *${count++}* ) Total RAM : ${sizeString(process.memoryUsage().rss)}\n`
      text += `( *${count++}* ) OS : ${process.platform + " " + process.arch}\n`
      text += `( *${count++}* ) Total Session : ${dir.length} Files\n`
      text += `( *${count++}* ) Size Session : ${sizeString(_b)}\n`
