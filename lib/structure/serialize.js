  ...
  constructor (message, sock, store) {
    ...
    if(this.message) {
      ...
      try {
        ...
        this.quoted.delete = () => sock.sendMessage(this.chat, { delete: this.quoted.key });
      ...
      this.editMessage = (text, keys) => {
        return sock.sendMessage(this.chat, { text, mentions: [this.sender, ...text.matchAll(/@([0-9]{5,16}|0)/g)].map((v) => v[1] + "@s.whatsapp.net"), edit: keys }, { quoted: this })
      };
      this.reply = (text, options = {}) => {
        return sock.sendMessage(this.chat, { text, mentions: [this.sender, ...text.matchAll(/@([0-9]{5,16}|0)/g)].map((v) => v[1] + "@s.whatsapp.net"), ...options }, { quoted: this, ...options })
      };
      this.download = (pathFile) => downloadMedia(this.message, pathFile);
      this.react = (text) => sock.sendMessage(this.chat, { react: { text, key: this.key } });
    };
    return this;
  };
};
